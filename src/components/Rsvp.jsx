import { useT, useLang } from '../i18n';
import { useAnimated } from '../context/AnimatedContext';

export default function Rsvp() {
  const t    = useT();
  const lang = useLang();
  const { coupleES, couple, whatsappNumber } = useAnimated();

  const waNumber = (whatsappNumber || '').replace(/\D/g, '');
  const waBase   = waNumber ? `https://wa.me/${waNumber}` : 'https://wa.me/';
  const waUrl    = `${waBase}?text=${encodeURIComponent(`Hola! Confirmo mi asistencia a la boda de ${coupleES}`)}`;
  const smsUrl   = `sms:?body=${encodeURIComponent(`Hi! I confirm my attendance to ${couple}'s wedding`)}`;

  const isEn = lang === 'en';
  const href = isEn ? smsUrl : waUrl;

  return (
    <section className="rsvp">
      <div className="rv">
        <h3>{t.rsvpTitle}</h3>
        <div className="big pf">{t.rsvpSubtitle}</div>
        <p>{t.rsvpText}</p>
        <a
          className="wa"
          href={href}
          target={isEn ? '_self' : '_blank'}
          rel="noopener noreferrer"
        >
          {isEn ? (
            <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
              <path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H6l-2 2V4h16v12z" />
            </svg>
          ) : (
            <svg viewBox="0 0 24 24" width="18" height="18">
              <path fill="currentColor" d="M17.5 14.4c-.3-.2-1.7-.8-2-.9-.3-.1-.5-.2-.7.2-.2.3-.7.9-.9 1.1-.2.2-.3.2-.6.1-1.7-.9-2.9-1.6-4-3.5-.3-.5.3-.5.8-1.5.1-.2 0-.4 0-.5 0-.2-.7-1.6-.9-2.2-.2-.6-.5-.5-.7-.5h-.6c-.2 0-.5.1-.8.4-.3.3-1 1-1 2.5s1.1 2.9 1.2 3.1c.1.2 2.1 3.3 5.2 4.6 2 .8 2.7.9 3.7.8.6-.1 1.7-.7 2-1.4.2-.7.2-1.2.2-1.4-.1-.1-.3-.2-.6-.3z" />
              <path fill="currentColor" d="M12 2a10 10 0 00-8.5 15.2L2 22l4.9-1.5A10 10 0 1012 2zm0 18a8 8 0 01-4.1-1.1l-.3-.2-3 .9.9-2.9-.2-.3A8 8 0 1112 20z" />
            </svg>
          )}
          {t.rsvpBtn}
        </a>
      </div>
    </section>
  );
}
